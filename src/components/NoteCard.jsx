    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-200">
      <div className="p-4">
        <div className="flex justify-between items-start mb-2">
          ...
          <button className={`text-sm flex item-center justify-cebter p-1 rounded-full transition-colors ${
        ...
        <div className="text-gray-700 text-sm mb-3 line-clamp-3 min-h-[3rem]">
          {note.content || <span className="text-gray-400 italic"> No conent </span>}
        </div>

        <div className="text-xs text-gray-500 mt-2">{formDate(note.createdAt)}</div>
      </div>
    </div>
  );
}
